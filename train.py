...
def train(gpu, args):
    ...
    N = args.n_frames # Number of frames: set to 7
    ...
    while should_keep_training:
        for i_batch, item in enumerate(train_loader):
            ...
            # fix first two camera poses to remove gauge freedom
            Gs.data[:,0] = Ps.data[:,0].clone()
            Gs.data[:,1:] = Ps.data[:,[1]].clone()
            disp0 = torch.ones_like(disps[:,:,3::8,3::8])

            # perform random restarts TODO why though?
            r = 0
            while r < args.restart_prob:
                ...
                # calculate pose loss, residual loss, and flow loss
                ...
                # weight poses, residual, and flow loss
